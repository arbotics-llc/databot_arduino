/*
 ...
 .trimAltitudeMeters(int user_altitude) allows user to trim altimeter with current altitude, optional
...
void MPL3115A2::trimAltitudeMeters(int userAltitude){
  //reset trim just incase board has not been powered off since reset
  IIC_Write(OFF_H, 0);
  readAltitude();
  delay(500);
  float sensorAltitude = readAltitude();
  //Serial.println(sensor_altitude);
  int difference = userAltitude - sensorAltitude;
  //Serial.print(F("altitude difference between user specified and sensor:"));
  ...
  //Serial.print(F("calculated trim value:"));
  //Serial.println((int)trimvalue);
  
  IIC_Write(OFF_H, trimvalue);
}

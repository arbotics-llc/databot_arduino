...
SHTC3_Status_TypeDef 	SHTC3::sendCommand(SHTC3_Commands_TypeDef cmd)
{
	...
	if(res){ return SHTC3_Status_Error; }
...
SHTC3_Status_TypeDef	SHTC3::checkID()
{
	...
	if(checkCRC(ID, IDcs) == SHTC3_Status_Nominal){ passIDcrc = true; }
...
SHTC3_Status_TypeDef 	SHTC3::update()
{
	...
	if(numBytesRx != numBytesRequest){ return abortUpdate(SHTC3_Status_Error, __FILE__, __LINE__); }		// Hopefully we got the right number of bytes
	...
	if(checkCRC(RH, RHcs) == SHTC3_Status_Nominal){ passRHcrc = true; }
	if(checkCRC(T, Tcs) == SHTC3_Status_Nominal){ passTcrc = true; }
...
SHTC3_Status_TypeDef SHTC3::checkCRC(uint16_t packet, uint8_t cs)
{
	...
	if(cs ^ crc){ return exitOp(SHTC3_Status_CRC_Fail, __FILE__, __LINE__); }
